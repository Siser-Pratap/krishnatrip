...
//   // if (menuToggle) {
//   //   menuToggle.addEventListener("click", toggleTimeline);
//   // }
...
//   })
  ...
  // return () => {
  //   if (menuToggle) {
  //     menuToggle.removeEventListener("click", toggleTimeline);
  //   }
  // };
// }, [tl]);
...
{!small&&(
      <div ref={fullPageNav} className='fullPageNav fixed left-0 top-0 w-[80%] h-[100vh] sm:hidden '>
      <div className='values flex flex-col gap-4 justify-start'>
            {navLinks.map((link, index)=>(<Link ref={navValues} className="mt-[0.3rem] navValues" key={index} href={`/${link}`}>{link}</Link>))}
